...
class UrbanSpider(scrapy.Spider):
	...
	def parse_members(self, response):
		def extract_with_css(query):
			return response.css(query).extract_first().strip().decode('utf-8','ignore')
		...
		try:
			...
			for e, var in enumerate(classesConducted):
				classesConducted[e] = var.strip().decode('utf-8','ignore').encode('utf-8')
		except:
		...
		try:
			...
			for var in review:
				...
				reviews.append(soup.get_text().strip())
		except:
			print "\n\nreviews\n\n",response,"\n\n"
			#inp=input()
